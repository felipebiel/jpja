        ...
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                ...
                @include('sistema.partes.mensagens')
                       ...
                       {!! Form::model($modulo, ['route' => ['sistema.modulos.atualizar', $modulo->id], 'method' =>'PUT','enctype' =>'multipart/form-data', 'class' => 'form-horizontal', 'id'=>'formModuloEditar' ]) !!}
...
$(document).ready(function(){
  ...
  var tableTemas = $('#tableTemas').DataTable({ //cria a datatable
        ...
        "oAria": {
         ...
         /*imagem no editar*/
$('[name="input_imagem_modulo"]').val('{{$modulo->imagem}}'); 
$("#imageModulo").css("background-image", "url('{{ url('arquivos/img_modulos').'/'.$modulo->imagem}}')");
...
$(document).on('click', '#sendModulo', function() {
    ...
    $.ajax({
        type:'post',
        url: '{{ url('sistema/modulos/atualizar')}}/'+{{$modulo->id}},
        dataType: 'json',
        data: $('#formModuloEditar').serialize(),
    })
    .done(function(data, textStatus, jqXHR){
        $('#formModuloEditar').submit();
    })
    .fail( function(jqXHR){
        if(jqXHR.status == 422){
            var  msg = " ";
